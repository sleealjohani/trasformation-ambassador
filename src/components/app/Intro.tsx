"use client";
/* eslint-disable @next/next/no-img-element -- العلامات SVG ثابتة داخل مشهد متحرك */
...
function IntroStage() {
  ...
  const dismiss = useCallback(() => { setComplete(true); window.setTimeout(() => setGone(true), FADE_MS); try { sessionStorage.setItem(SEEN_KEY, "1"); } catch { /* وضع خاص */ } }, []);

  /* eslint-disable react-hooks/set-state-in-effect -- حالة جلسة الجهاز لا تتوفر على الخادم */
  useEffect(() => { let seen = false; try { seen = sessionStorage.getItem(SEEN_KEY) === "1"; } catch { seen = false; } if (seen) { setShow(false); setGone(true); return; } if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) { setReduced(true); setPhase("hold"); } }, []);
  ...
  useEffect(() => {
    ...
    else { for (const [next, at] of TIMELINE) timers.current.push(window.setTimeout(() => setPhase(next), at)); const last = TIMELINE[TIMELINE.length - 1]; timers.current.push(window.setTimeout(dismiss, (last?.[1] ?? 3550) + HOLD_MS)); }
    const ids = timers.current;
    return () => { ids.forEach(window.clearTimeout); document.documentElement.style.overflow = ""; };
  }, [show, gone, reduced, dismiss]);
  if (gone) return null;

  return <div className="intro" data-phase={phase} data-complete={complete ? "true" : "false"} role="status" aria-live="polite" aria-label="مقدمة جسر التحول">
    <div className="intro__bg" aria-hidden /><div className="intro__grain" aria-hidden />
    <div className="intro__controls"><SoundToggle className="intro__btn" showText /><button type="button" className="intro__btn" onClick={dismiss}>تخطٍّ</button></div>
    <div className="intro__stage" aria-hidden>
      <div className="intro__layer intro__layer--old"><div className="intro__crop intro__crop--old-symbol"><img src="/brand/moh-mark.svg" alt="" draggable={false} /></div><div className="intro__crop intro__crop--old-word"><img src="/brand/moh-mark.svg" alt="" draggable={false} /></div></div>
      <svg className="intro__geo" viewBox="0 0 1000 760" preserveAspectRatio="xMidYMid meet" aria-hidden><defs><filter id="introSoft" x="-80%" y="-80%" width="260%" height="260%"><feGaussianBlur stdDeviation="5" result="b" /><feMerge><feMergeNode in="b" /><feMergeNode in="SourceGraphic" /></feMerge></filter><filter id="introCore" x="-100%" y="-100%" width="300%" height="300%"><feGaussianBlur stdDeviation="12" result="b1" /><feGaussianBlur stdDeviation="3" result="b2" /><feMerge><feMergeNode in="b1" /><feMergeNode in="b2" /><feMergeNode in="SourceGraphic" /></feMerge></filter><linearGradient id="introGold" x1="0" y1="0" x2="1" y2="0"><stop offset="0" stopColor="#a69b62" /><stop offset=".5" stopColor="#d3c88a" /><stop offset="1" stopColor="#5fcfd0" /></linearGradient><linearGradient id="introGreen" x1="0" y1="0" x2="1" y2="0"><stop offset="0" stopColor="#009b72" /><stop offset=".55" stopColor="#27c3a4" /><stop offset="1" stopColor="#59c8ef" /></linearGradient><linearGradient id="introPetal" x1=".25" y1="0" x2=".75" y2="1"><stop offset="0" stopColor="#35c6a7" /><stop offset=".45" stopColor="#57d6de" /><stop offset="1" stopColor="#edf8ff" /></linearGradient><radialGradient id="introCoreFill"><stop offset="0" stopColor="#f8ffff" stopOpacity="1" /><stop offset=".22" stopColor="#79e0db" stopOpacity=".95" /><stop offset=".58" stopColor="#26a9d8" stopOpacity=".48" /><stop offset="1" stopColor="#26a9d8" stopOpacity="0" /></radialGradient></defs>
    ...
    </div><p className="intro__promise">{STRINGS.motto}</p>
  </div>;
}
