...
export function SiteAudioProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (muted || mediaMode || document.visibilityState === "hidden") {
      audio.pause();
      return;
    }
    ...
    let cancelled = false;
    const attempt = () => {
      if (cancelled || muted || mediaMode) return;
      audio.volume = AMBIENT_VOLUME;
  ...
  useEffect(() => {
    const onVisibility = () => {
      const audio = audioRef.current;
      if (!audio) return;
      if (document.visibilityState === "hidden" || muted || mediaMode) {
        audio.pause();
      } else {
        audio.volume = AMBIENT_VOLUME;
        void audio.play().catch(() => undefined);
  ...
  return (
    <SiteAudioContext.Provider value={value}>
      <audio ref={audioRef} data-site-ambient src="/sound/anthem.mp3" preload="auto" loop playsInline />
      {children}
...
export function SoundToggle({ className, showText = false }: { className?: string; showText?: boolean }) {
  ...
  return (
    <button type="button" onClick={toggle} aria-label={label} aria-pressed={!muted} title={help} className={cn(className ?? "v2-sound-toggle", "inline-flex items-center justify-center gap-2")}>
      <Icon name={muted || mediaMode ? "mute" : "volume"} size={17} />
