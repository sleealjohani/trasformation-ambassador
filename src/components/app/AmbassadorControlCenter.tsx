...
export function AmbassadorControlCenter({ topics, initialInbox, initialIssues, initialSnapshot }: WorkProps & { initialSnapshot: ControlSnapshot }) {
  ...
  return <div className="flex flex-col gap-5">
    <div className="sticky top-0 z-20 -mx-4 border-b border-line bg-bg/95 px-4 py-3 backdrop-blur md:mx-0 md:rounded-card md:border md:bg-panel md:p-2">
      <div className="flex items-center gap-2 overflow-x-auto" role="tablist" aria-label="أقسام مركز التحكم">{NAV.map((item) => <button key={item.key} type="button" role="tab" aria-selected={tab === item.key} onClick={() => setTab(item.key)} className={tab === item.key ? "inline-flex min-h-11 shrink-0 items-center gap-2 rounded-button bg-hh-2736 px-4 text-secondary font-bold text-panel" : "inline-flex min-h-11 shrink-0 items-center gap-2 rounded-button px-4 text-secondary text-ink-soft hover:bg-panel-2"}><Icon name={item.icon} size={16} /><span>{item.label}</span></button>)}<button type="button" onClick={() => void refresh()} disabled={refreshing} className="ms-auto inline-flex min-h-11 shrink-0 items-center gap-2 rounded-button px-3 text-secondary text-muted hover:bg-panel-2"><Icon name="retry" size={15} />{refreshing ? "تحديث…" : "تحديث"}</button></div>
    </div>
